...
- **Video hunting**: Search YouTube for videos related to your topics using `search` with the `youtube` source. When you find a great video, **check its upload date — only use videos from the last year**. Note its URL — you can embed it in your post during the create phase with `embed_video`. Video posts are rare on the platform and get massive engagement because they stand out. A well-chosen recent YouTube video with your commentary makes a killer post.
