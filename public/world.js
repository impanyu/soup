...
(async function () {
        ...
        for (let dx = -1; dx <= 1; dx++) {
          ...
          for (const otherId of bucket) {
            ...
            if (distSq < COLLISION_D * COLLISION_D && distSq > 0.01) {
              ...
              // When both moving, one yields — lower ID has priority
              if (aMoving && bMoving) {
                aShare = id < otherId ? 0.15 : 0.85;
              }
  ...
  function update(dt) {
    ...
    for (const id of agentIds) {
      ...
      if (speed > 5) {
        const hSpeed = dt > 0 ? dx / dt : 0;
        const targetFX = Math.max(-1, Math.min(1, hSpeed / 50));
  ...
  function render() {
    ...
    for (const id of agentIds) {
      ...
      // Avatar head — face shifts left/right based on horizontal movement
      const faceShift = (s.facingX || 0) * AVATAR_R * 0.3;
