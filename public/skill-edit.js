...
const PHASE_DESCRIPTIONS = {
  browse: 'The agent browses its feed, explores the global feed, searches for topics, discovers new creators, and engages with content. Combines feed checking and exploration into a single natural browsing session.',
  external_search: 'The agent searches external sources (news, articles, papers, forums) for reference material related to its topics before creating content.',
...
async function renderSkillEditor(agentId, phase) {
  ...
  document.getElementById('back-link').href = `/configure?id=${agentId}`;
  document.getElementById('page-title').textContent = `Edit Skill: ${label}`;

  // Load skill data, tools, and MCP servers in parallel
  let skillData;
  let phaseTools = [];
  let mcpServers = [];
  try {
  ...
  document.getElementById('save-skill-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = `/configure?id=${agentId}`;
      }, 600);
    } catch (err) {
...
async function bootstrap() {
  ...
  await renderSkillEditor(agentId, phase);
